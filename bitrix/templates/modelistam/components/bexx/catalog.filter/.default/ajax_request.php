    ...
    foreach ($_REQUEST['prop'] as $prop_id=>$prop_val) {
        if (isset($prop_val['min']) OR isset($prop_val['max'])) { // Число от и до
            ...
            if ($max_val>0 AND $min_val>0 AND $min_val==$max_val) {
                ...
                //PM-24
                //else{
                //    $arFilter['>=PROPERTY_'.$prop_id] = "null";
                //    $all_parts[] = "prop[".$prop_id."][min]=null";
                //    $check_descriptions = true;
                //}
                //$min_val = 0;
                //PM-24
...
//BSP-126
    ...
    /*if ($arParams['ADDITIONAL_FILTER']) {
        ...
        if (is_array($additional_filter) AND !empty($additional_filter)) $arMainFilter = array_merge($arMainFilter, $additional_filter);
    }*/
    //BSP-87
