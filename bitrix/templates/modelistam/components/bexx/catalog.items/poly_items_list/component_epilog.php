...
<script>
    ...
    <?if (is_array($GLOBALS['cart_content'])):?>
        <?foreach ($GLOBALS['cart_content'] as $item_id=>$odin):?>
            <?if (isset($arResult['ITEMS'][$item_id])):?>
                $('.wrap-order-<?=$arParams['CODE']?>').find('#product-order-<?=$item_id?>').html('<a href="/personal/cart/" class="butt1 buttCartActive">В корзине</a>').parents('.poly-items__bottom').addClass('poly-items__bottom--active');
            <?endif;?>
        <?endforeach;?>
    <?endif;?>
    ...
    <?if (is_array($GLOBALS['wishlist_content'])):?>
        <?foreach ($GLOBALS['wishlist_content'] as $item_id=>$odin):?>
            <?if (isset($arResult['ITEMS'][$item_id])):?>
                $('#poly-items-<?=$arParams['CODE']?>').find('#product-wishlist-<?=$item_id?>').html('<a class="wishlist-link wishlist-link--active" href="/personal/">Отложено</a>');
            <?endif;?>
        <?endforeach;?>
    <?endif;?>
